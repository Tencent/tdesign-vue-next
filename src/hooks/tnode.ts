import { h, getCurrentInstance, VNode, Slots, isVNode } from 'vue';
...
/**
 * 用于组合式的TNodeJSX，在组合时逻辑时渲染 TNode，props 和 插槽同时处理，也能处理默认值为 true 则渲染默认节点的情况，与render-tnode的大概处理逻辑相同。
 * @param name 插槽和属性名称
...
/**
 * 在组合逻辑时，通过JSX的方式 TNode，props 和 插槽同时处理。与 renderTNodeJSX 区别在于属性值为 undefined 时会渲染默认节点
 * @param name 插槽和属性名称
