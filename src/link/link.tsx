    ...
    return () => {
      ...
      return (
        <a
          ...
          onClick={handleClick}
        >
          {prefixIcon ? <span class={`${COMPONENT_NAME.value}__prefix-icon`}>{prefixIcon}</span> : null}
          {linkContent}
          {suffixIcon ? <span class={`${COMPONENT_NAME.value}__suffix-icon`}>{suffixIcon}</span> : null}
        </a>
      );
    };
  },
