...
Dialog 组件在 1.0.8 之后使用 Vue3 的 [Teleport](https://cn.vuejs.org/guide/built-ins/teleport.html) 重构了 `attach` 属性的实现，因此 `:deep()` 深度选择器无法作用于 [Teleport](https://cn.vuejs.org/guide/built-ins/teleport.html)。如果您想自定义主题，我们更推荐使用替换 CSS Variables 的方式来进行个性化的主题配置。具体请参见 `自定义主题配置` 菜单。
