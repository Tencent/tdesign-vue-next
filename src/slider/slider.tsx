import { defineComponent, reactive, VNode, computed, ref, onMounted, onBeforeUnmount, watch, toRefs } from 'vue';
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    /** ----------------- 样式计算 start ------------- */
    const containerClass = computed(() => [`${name}__container`, { 'is-vertical': vertical.value }]);
    const sliderClass = computed(() => {
      return [
        `${name}`,
        {
          'is-vertical': vertical.value,
          [`${name}--with-input`]: props.inputNumberProps,
          [`${name}--vertical`]: vertical.value,
    ...
    //   if (props.range) {
    //     return ![minValue.value, maxValue.value].every((item, index) => item === sliderState.prevValue[index]);
    //   }
    //   return props.value !== sliderState.prevValue;
    // };

    // tips: 没发现有调用该函数的逻辑
    // const setInputProps = (): void => {
    //   if (typeof props.inputNumberProps !== 'boolean') {
    //     const inputNumbeConfig = props.inputNumberProps as TdSliderProps['inputNumberProps'];
    ...
    //       sliderInputState.inputDecimalPlaces = inputDecimalPlaces;
    //     }
    //     if (inputPlaceholder) {
    //       sliderInputState.inputPlaceholder = inputPlaceholder;
    //     }
    //     if (typeof inputFormat === 'function') {
    //       sliderInputState.inputFormat = inputFormat;
    //     }
    //     if (['column', 'row', 'normal'].includes(inputTheme)) {
    //       sliderInputState.inputTheme = inputTheme;
    //     }
    //   }
    // };
    ...
    const { renderMask } = useSliderMark(props.max, props.min, props.marks, vertical.value, name);

    const { renderInputNumber } = useSliderInput(props.max, props.min, props.step, name, vertical.value, disabled);
    ...
    const renderInputButton = (): VNode => {
      ...
      return (
        ...
        >
          ...
          {props.range && <div class={`${name}__center-line`} />}
        ...
        <div class={containerClass.value} ref={sliderContainerRef}>
          ...
          >
            <div class={sliderRailClass.value} style={runwayStyle.value} onClick={onSliderClick} ref={sliderRef}>
              <div class={`${name}__track`} style={barStyle.value} />
              <t-slider-button
              ...
              {sliderState.showSteps && (
                <div>
                  {steps.value.map((item, key) => (
                    <div class={`${name}__stop`} key={key} style={getStopStyle(item, vertical.value)} />
                  ))}
                </div>
              )}
              {MASKS}
