...
export default {
  before({ source, file }) {
    ...
    if (fileName && source.includes(':: BASE_DOC ::')) {
      const localeDocPath = path.resolve(__dirname, '../../', `node_modules/@td/shared/_common/docs/web/api/${fileName}`);
      const defaultDocPath = path.resolve(__dirname, '../../', `node_modules/@td/shared/_common/docs/web/api/${componentName}.md`);
      let baseDoc = '';
