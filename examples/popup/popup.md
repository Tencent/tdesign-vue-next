...
overlayStyle | Boolean / Object / Function | - | 浮层样式。TS 类型：`Styles | ((trigger: HTMLElement) => Styles)`。[通用类型定义](https://github.com/Tencent/tdesign-vue-next/blob/develop/src/common.ts) | N
...
visible | Boolean | false | 是否显示浮层。支持语法糖 `v-model:visible`。TS 类型：`boolean` | N
