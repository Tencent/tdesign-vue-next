const componentDocs = [
  {
    title: 'Chatbot 智能对话',
    titleEn: 'Chatbot',
    name: 'Chatbot',
    path: '/vue-next-chat/components/chatbot',
    component: () => import('@tdesign/pro-components-chat/chatbot/chatbot.md'),
  },
  {
    title: 'ChatEngine 对话引擎',
    titleEn: 'ChatEngine',
    name: 'ChatEngine',
    path: '/vue-next-chat/components/chat-engine',
    component: () => import('@tdesign/pro-components-chat/chat-engine/chat-engine.md'),
  },
  {
    title: 'ChatSender 对话输入',
    titleEn: 'ChatSender',
    name: 'ChatSender',
    path: '/vue-next-chat/components/chat-sender',
    component: () => import('@tdesign/pro-components-chat/chat-sender/chat-sender.md'),
  },
  {
    title: 'ChatMessage 对话消息体',
    titleEn: 'ChatMessage',
    name: 'ChatMessage',
    path: '/vue-next-chat/components/chat-message',
    component: () => import('@tdesign/pro-components-chat/chat-message/chat-message.md'),
  },
  {
    title: 'ChatActionbar 对话操作栏',
    titleEn: 'ChatActionbar',
    name: 'ChatActionbar',
    path: '/vue-next-chat/components/chat-actionbar',
    component: () => import('@tdesign/pro-components-chat/chat-actionbar/chat-actionbar.md'),
  },
  {
    title: 'ChatMarkdown Markdown内容',
    titleEn: 'ChatMarkdown',
    name: 'ChatMarkdown',
    path: '/vue-next-chat/components/chat-markdown',
    component: () => import('@tdesign/pro-components-chat/chat-markdown/chat-markdown.md'),
  },
  {
    title: 'ChatThinking 思考过程',
    titleEn: 'ChatThinking',
    name: 'ChatThinking',
    path: '/vue-next-chat/components/chat-thinking',
    component: () => import('@tdesign/pro-components-chat/chat-thinking/chat-thinking.md'),
  },
  {
    title: 'ChatLoading 对话加载',
    titleEn: 'ChatLoading',
    name: 'ChatLoading',
    path: '/vue-next-chat/components/chat-loading',
    component: () => import('@tdesign/pro-components-chat/chat-loading/chat-loading.md'),
  },
  {
    title: 'Attachments 文件附件',
    titleEn: 'Attachments',
    name: 'Attachments',
    path: '/vue-next-chat/components/attachments',
    component: () => import('@tdesign/pro-components-chat/attachments/attachments.md'),
  },
  {
    title: 'ChatList 对话列表',
    titleEn: 'ChatList',
    name: 'ChatList',
    path: '/vue-next-chat/components/chat-list',
    component: () => import('@tdesign/pro-components-chat/chat-list/chat-list.md'),
  },
  {
    title: 'ChatContent 对话正文',
    titleEn: 'ChatContent',
    name: 'ChatContent',
    path: '/vue-next-chat/components/chat-content',
    component: () => import('@tdesign/pro-components-chat/chat-content/chat-content.md'),
  },
  {
    title: 'ChatItem 对话单元',
    titleEn: 'ChatItem',
    name: 'ChatItem',
    path: '/vue-next-chat/components/chat-item',
    customTag: '待废弃',
    component: () => import('@tdesign/pro-components-chat/chat-item/chat-item.md'),
  },
  {
    title: 'ChatInput 对话输入',
    titleEn: 'ChatInput',
    name: 'ChatInput',
    path: '/vue-next-chat/components/chat-input',
    customTag: '待废弃',
    component: () => import('@tdesign/pro-components-chat/chat-input/chat-input.md'),
  },
  {
    title: 'ChatReasoning 思考过程',
    titleEn: 'ChatReasoning',
    name: 'ChatReasoning',
    path: '/vue-next-chat/components/chat-reasoning',
    customTag: '待废弃',
    component: () => import('@tdesign/pro-components-chat/chat-reasoning/chat-reasoning.md'),
  },
];

export const docs = [
  {
    title: '开始',
    titleEn: 'Start',
    type: 'doc',
    children: [
      {
        title: '概述',
        titleEn: 'Overview',
        name: 'overview',
        path: '/vue-next-chat/overview',
        component: () => import('./docs/intro.md'),
      },
      {
        title: '快速开始',
        titleEn: 'Getting Started',
        name: 'getting-started',
        path: '/vue-next-chat/getting-started',
        component: () => import('./docs/getting-started.md'),
      },
      {
        title: '更新日志',
        titleEn: 'CHANGELOG',
        name: 'changelog',
        path: '/vue-next-chat/changelog',
        component: () => import('../CHANGELOG.md'),
      },
      {
        title: '什么是流式输出',
        name: 'sse',
        path: '/vue-next-chat/sse',
        component: () => import('./docs/sse.md'),
      },
      {
        title: '与AG-UI协议集成',
        titleEn: 'AG-UI',
        name: 'agui',
        path: '/vue-next-chat/agui',
        component: () => import('./docs/agui.md'),
      },
    ],
  },
  {
    title: '全局配置',
    titleEn: 'Global Config',
    type: 'doc',
    children: [
      {
        title: '自定义主题',
        titleEn: 'Theme Customization',
        name: 'custom-theme',
        path: '/vue-next-chat/custom-theme',
        component: () => import('@tdesign/common/docs/web/theme.md'),
        componentEn: () => import('@tdesign/common/docs/web/theme.en-US.md'),
      },
      {
        title: '深色模式',
        titleEn: 'Dark Mode',
        name: 'dark-mode',
        path: '/vue-next-chat/dark-mode',
        component: () => import('@tdesign/common/docs/dark-mode.md'),
        componentEn: () => import('@tdesign/common/docs/dark-mode.en-US.md'),
      },
      {
        title: '自定义样式',
        titleEn: 'Style Customization',
        name: 'custom-style',
        path: '/cha/custom-style',
        component: () => import('./docs/style.md'),
      },
    ],
  },
  {
    title: '智能对话',
    titleEn: 'Data Display',
    type: 'component',
    children: componentDocs,
  },
];

export default { docs };
