<template>
  ...
  <ul v-else class="empty-demo">
    <li v-for="demoName in demoList[componentName] || []" :key="demoName">
      <router-link :to="{ path: `/vue-next-chat/demos/${componentName}/${demoName}` }">
...
<script>
const demoReq = import.meta.glob('@tdesign/pro-components-chat/**/_example/*.{vue,jsx,tsx}', { eager: true });
const demoObject = {};
const demoList = {};

Object.keys(demoReq).forEach((key) => {
  const match = key.match(/([\w-]+)\/_example\/([\w-]+)\.(vue|jsx|tsx)$/);
  if (!match) return;
