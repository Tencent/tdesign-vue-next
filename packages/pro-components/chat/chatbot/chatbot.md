...
| messageProps      | Object/Function | -      | 消息项配置，透传给内部 [ChatMessage](/chat/components/chat-message) 组件。按角色聚合了消息项的配置，可以是静态配置对象或动态配置函数。TS 类型：`TdChatMessageConfig \| ((msg: ChatMessagesData) => TdChatMessageConfigItem)` ，[详细类型定义](https://github.com/TDesignOteam/tdesign-web-components/blob/develop/src/chatbot/type.ts#L25) | N    |
