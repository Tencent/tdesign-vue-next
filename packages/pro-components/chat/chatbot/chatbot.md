...
| messageProps      | Object/Function | -      | 消息项配置，透传给内部 [ChatMessage](/chat/components/chat-message) 组件。按角色聚合了消息项的配置，可以是静态配置对象或动态配置函数。TS 类型：`TdChatMessageConfig \| ((msg: ChatMessagesData) => TdChatMessageConfigItem)` （来自 `@tdesign/web-components-chat`） | N    |
...
| chatServiceConfig | Object          | -      | 对话服务配置，包含网络请求和回调配置，见下方详细说明，TS 类型：`ChatServiceConfig`（来自 `@tdesign/web-components-chat`）                                                                                                                                                                                                                   | N    |
