...
panelContentBottom | Slot / Function | - | 面板内容底部（每一列的底部内容），支持自定义渲染。`panelIndex` 表示当前列索引，`options` 表示当前列原始选项，`filteredOptions` 表示当前列过滤后的选项（未过滤时与 options 相同），`onFilter` 用于过滤当前列选项（当传入字符串时，内置过滤为大小写不敏感匹配；如需大小写敏感匹配，请传入自定义过滤函数）。注意：当内置搜索功能（filterable）激活时，此插槽不会渲染。TS 类型：`TNode<{ panelIndex: number; options: TreeOptionData[]; filteredOptions: TreeOptionData[]; onFilter: (filter: string \| ((node: TreeOptionData, panelIndex: number) => boolean), opts?: { cascade?: boolean }) => void }>`。[通用类型定义](https://github.com/Tencent/tdesign-vue-next/blob/develop/packages/components/common.ts) | N
panelContentTop | Slot / Function | - | 面板内容顶部（每一列的顶部内容），支持自定义渲染。`panelIndex` 表示当前列索引，`options` 表示当前列原始选项，`filteredOptions` 表示当前列过滤后的选项（未过滤时与 options 相同），`onFilter` 用于过滤当前列选项（当传入字符串时，内置过滤为大小写不敏感匹配；如需大小写敏感匹配，请传入自定义过滤函数）。注意：当内置搜索功能（filterable）激活时，此插槽不会渲染。TS 类型：`TNode<{ panelIndex: number; options: TreeOptionData[]; filteredOptions: TreeOptionData[]; onFilter: (filter: string \| ((node: TreeOptionData, panelIndex: number) => boolean), opts?: { cascade?: boolean }) => void }>`。[通用类型定义](https://github.com/Tencent/tdesign-vue-next/blob/develop/packages/components/common.ts) | N
