<template>
  <t-space direction="vertical" class="tdesign-demo__select-input-multiple" style="width: 100%">
    ...
    <t-select-input
      ...
      :on-tag-change="
        () => {
          console.log('tag change on top');
        }
      "
      :tag-input-props="{
        excessTagsDisplayType,
        onChange: () => {
          console.log('55555555 taginput change');
        },
      }"
      :popup-props="popupProps"
      clearable
      :on-focus="() => console.log('select input focus')"
      :on-change="() => console.log('select input change')"
      :input-props="{
        placeholder: '粘贴多个标签（逗号分隔）',
        size: 'large',
...
const onTagChange = (currentTags, context) => {
  console.log('nb', currentTags, context);
