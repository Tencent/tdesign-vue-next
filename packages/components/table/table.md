...
onColumnControllerVisibleChange | Function |  | TS 类型：`(visible: boolean, context: { trigger: 'cancel' \| 'confirm' | 'open' }) => void`<br/>列配置弹窗显示或隐藏变化时触发。`visible` 表示弹窗是否可见；`context.trigger` 表示触发来源：`'open'` 表示打开弹窗，`'confirm'` 表示点击确认按钮关闭，`'cancel'` 表示点击取消按钮或其他方式关闭 | N
