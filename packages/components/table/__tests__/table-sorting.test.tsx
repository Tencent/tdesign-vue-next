    ...
    SORTABLE_COMPONENTS.forEach(({ name, component: TableComponent }) => {
      describe(`${name}`, () => {
        ...
        it('should sort data in ascending order when clicked once', async () => {
          ...
          const idHeader = wrapper.findAll('thead th')[0];
          expect(
            idHeader.find('.t-table__sort-icon--active').exists() ||
              idHeader.find('.t-table-sort-asc').exists() ||
              idHeader.classes().some((cls) => cls.includes('asc')),
          ).toBeTruthy();
        });
        ...
        it('should sort data in descending order when clicked twice', async () => {
          ...
          expect(
            idHeader.find('.t-table__sort-icon--active').exists() ||
              ...
              idHeader.classes().some((cls) => cls.includes('desc')),
          ).toBeTruthy();
        });
    ...
    SORTABLE_COMPONENTS.forEach(({ name, component: TableComponent }) => {
      describe(`${name} - Custom Sorters`, () => {
        ...
        it('should handle numeric sorting correctly', async () => {
          ...
          const wrapper = mount(() => <TableComponent data={mockData} columns={numericColumns} rowKey="id" />);
        ...
        it('should handle boolean sorting', async () => {
          ...
          const wrapper = mount(() => <TableComponent data={booleanData} columns={booleanColumns} rowKey="id" />);
    ...
    SORTABLE_COMPONENTS.forEach(({ name, component: TableComponent }) => {
      describe(`${name} - Controlled Sort`, () => {
        ...
        it('should respect external sort changes', async () => {
          ...
          const wrapper = mount(() => (
            <TableComponent data={mockData} columns={sortableColumns} rowKey="id" sort={sort.value} />
          ));
    ...
    SORTABLE_COMPONENTS.forEach(({ name, component: TableComponent }) => {
      describe(`${name} - Multi-column Sort`, () => {
        it('should handle multiple sort criteria', async () => {
          ...
          const wrapper = mount(() => (
            <TableComponent data={multiSortData} columns={multiSortColumns} rowKey="id" onSortChange={onSortChange} />
          ));
    ...
    SORTABLE_COMPONENTS.forEach(({ name, component: TableComponent }) => {
      describe(`${name} - Edge Cases`, () => {
        it('should handle empty data gracefully', async () => {
          const wrapper = mount(() => <TableComponent data={[]} columns={sortableColumns} rowKey="id" />);
        ...
        it('should handle null and undefined values', async () => {
          ...
          const wrapper = mount(() => <TableComponent data={nullableData} columns={nullableColumns} rowKey="id" />);
