...
pixelStyle | Object | - | Typescript: `QRCodePixelStyle` `type QRCodePixelStyle = { shape?: QRCodePixelShape; scale?: number}` `type QRCodePixelShape = 'square' \| 'mini-square' \| 'rounded' \| 'dot'`。[see more ts definition](https://github.com/Tencent/tdesign-vue-next/blob/develop/packages/components/qrcode/type.ts) | N
