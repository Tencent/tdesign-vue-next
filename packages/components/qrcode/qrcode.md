...
pixelStyle | Object | - | 二维码风格。`shape`默认值为`square`，可选值：`square`、`mini-square`、`rounded`、`dot`。`scale`仅在 `shape`为 `mini-square` 或 `dot` 时生效。`scale`值有效范围为`[0, 100]`。TS 类型：`QRCodePixelStyle` `type QRCodePixelStyle = { shape?: QRCodePixelShape; scale?: number}` `type QRCodePixelShape = 'square' \| 'mini-square' \| 'rounded' \| 'dot'`。[详细类型定义](https://github.com/Tencent/tdesign-vue-next/blob/develop/packages/components/qrcode/type.ts) | N
