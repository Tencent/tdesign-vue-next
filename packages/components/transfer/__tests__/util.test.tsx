...
import {
  ...
  isTreeNodeValid,
  findTopNode,
} from '@tdesign/components/transfer/utils';

// 测试数据工厂函数
const createTreeNode = (value: string, label: string, children?: any[]) => ({
  value,
  label,
  ...(children && { children }),
});
...
describe('Transfer Utils', () => {
  describe('getDataValues', () => {
    ...
    it('should skip null/undefined items in data', () => {
      const data = [{ value: 'a', label: 'A' }, null, undefined, { value: 'b', label: 'B' }];
      const checked = ['a', 'b', 'c'];
      const result = getDataValues(data, checked);
              ...
              children: [
                {
                  value: '1.2.1',
                  label: '1.2.1',
                  children: [{ value: '1.2.1.1', label: '1.2.1.1' }],
                },
                {
                  value: '1.2.2',
                  label: '1.2.2',
                  children: [{ value: '1.2.2.1', label: '1.2.2.1' }],
                },
              ],
            },
          ],
  ...
  describe('getLefCount', () => {
    ...
    it('should count leaf nodes correctly for complex tree', () => {
      expect(getLefCount(treeData)).toBe(10);
    });
...
describe('findTopNode', () => {
  ...
  it('should return self if name is t-transfer', () => {
    const vm = { $options: { name: 't-transfer' } };
  ...
  it('should return top node with name t-transfer', () => {
    ...
    expect(findTopNode(vm)).toBe(tTransferVm);
  });
  it('should return root if no t-transfer found', () => {
    const vm = createVmTree(['child', 'parent', 'root']);
    const rootVm = vm.$parent.$parent;
    expect(findTopNode(vm)).toBe(rootVm);
  });
});

describe('isTreeNodeValid', () => {
  it('should return !needMatch when data is null', () => {
    expect(isTreeNodeValid(null, ['a', 'b'], true)).toBe(false);
    expect(isTreeNodeValid(null, ['a', 'b'], false)).toBe(true);
  });
  it('should return !needMatch when data is undefined', () => {
    expect(isTreeNodeValid(undefined, ['a', 'b'], true)).toBe(false);
    expect(isTreeNodeValid(undefined, ['a', 'b'], false)).toBe(true);
  });
});
