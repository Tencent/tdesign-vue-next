...
import { useTNodeJSX, useConfig, usePrefixClass, useCommonClassName } from '@td/adapter-hooks';

import TLoading from '../loading';
import TListItem from './list-item';

import props from '@td/intel/components/list/props';
          ...
          isVirtualScroll ? (
            [
              ...
              <ul class={`${COMPONENT_NAME.value}__inner`} style={listStyle.value}>
                {
                  virtualConfig.visibleData.value.map(
                    (item) => <TListItem v-slots={item.slots} {...omit(item, 'slots')}></TListItem>
                  )
                }
              </ul>
            ]
