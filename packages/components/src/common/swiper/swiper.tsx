import { defineComponent, ref, computed, watch, isVNode, onMounted, cloneVNode, toRefs } from '@td/adapter-vue';
import { ChevronLeftIcon as TdChevronLeftIcon, ChevronRightIcon as TdChevronRightIcon } from 'tdesign-icons-vue-next';

import { usePrefixClass, useGlobalIcon, useChildComponentSlots, useTNodeJSX, useVModel } from '@td/adapter-hooks';
import props from '@td/intel/components/swiper/props';
import { SwiperNavigation, SwiperChangeSource } from '@td/intel/components/swiper/type';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const [innerCurrent, setInnerCurrent] = useVModel(current, current, props.defaultCurrent, props.onChange)
    ...
    const currentIndex = ref(innerCurrent.value || props.defaultCurrent);
    const navActiveIndex = ref(innerCurrent.value || props.defaultCurrent);
    ...
    const swiperTo = (index: number, context: { source: SwiperChangeSource }) => {
      ...
      setInnerCurrent(targetIndex, context)
    ...
    watch(
      innerCurrent,
      () => {
        innerCurrent.value !== undefined && swiperTo(innerCurrent.value, { source: 'autoplay' });
      },
    );
