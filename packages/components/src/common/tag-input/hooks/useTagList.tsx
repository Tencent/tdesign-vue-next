...
import type { TagInputValue, TagInputChangeContext } from '@td/intel/components/tag-input/type';
import { useVModel, usePrefixClass, useTNodeJSX, useEmitEvent } from '@td/adapter-hooks';
...
export default function useTagList(props: TagInputProps) {
  ...
  const { value, modelValue, onRemove, max, minCollapsedNum, size, disabled, readonly, tagProps, getDragProps } = toRefs(props);
  
  // handle controlled property and uncontrolled property
  const [tagValue, setTagValue] = useVModel(value, modelValue, props.defaultValue || [], props.onChange);
  console.log(tagValue.value, 'value');
  ...
  const onInnerEnter = (value: string, context: { e: KeyboardEvent }) => {
    ...
    emitEvent('enter', newValue, { ...context, inputValue: value })
  };
