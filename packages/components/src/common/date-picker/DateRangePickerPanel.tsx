...
} from '@td/intel/components/date-picker/type';
...
import dateRangePickerPanelProps from '@td/intel/components/date-picker/date-range-picker-panel-props';
import dateRangePickerProps from '@td/intel/components/date-picker/date-range-picker-props';
  ...
  setup(props: TdDateRangePickerPanelProps) {
    ...
    function onMonthChange(nextVal: number, { partial }: { partial: DateRangePickerPartial }) {
      ...
      if (year.value[0] === year.value[1]) {
        if (partialIndex === 0) {
          nextMonth[1] = Math.max(...nextMonth);
        }
        if (partialIndex === 1) {
          nextMonth[0] = Math.min(...nextMonth);
        }
      }
