import type { Ref, Slots, VNode } from '@td/adapter-vue';
...
import type { SelectOption, SelectOptionGroup, SelectValue, TdOptionProps, TdSelectProps } from '@td/intel/components/select/type';
import type { KeysType } from '@td/shared/interface';
...
export function useSelectOptions(props: TdSelectProps, keys: Ref<KeysType>, inputValue: Ref<string>) {
  ...
  const displayOptions = computed(() => {
    ...
    } // 远程搜索时，不执行内部的过滤，不干预用户的自行处理，如输入首字母搜索中文的场景等

    if (!inputValue.value || !(props.filterable || isFunction(props.filter))) {
      return options.value;
    }
