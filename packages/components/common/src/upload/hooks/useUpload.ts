...
import type { SizeLimitObj, TdUploadProps, UploadChangeContext, UploadFile, UploadRemoveContext } from '@td/components/upload/type';
import {
  formatToUploadFile,
  getDisplayFiles,
  getFilesAndErrors,
...
export default function useUpload(props: TdUploadProps) {
  ...
  const onFileChange = (files: File[]) => {
    ...
    validateFile({
      ...
      if (Array.isArray(args.fileValidateList)) {
  ...
  function uploadFiles(toFiles?: UploadFile[]) {
    ...
    upload({
      ...
      onResponseError,
      setXhrObject: (xhr) => {
        if (xhr.files[0]?.raw && xhrReq.value.find(item => item.files[0]?.raw === xhr.files[0].raw)) {
          return;
        }
